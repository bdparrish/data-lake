...
func (converter JsonMarshallingConverter[proto, db]) DbToProto(source *db) (*proto, error) {
	var response proto
...
func (converter JsonMarshallingConverter[proto, db]) ProtoToDb(source *proto) (*db, error) {
	var response db
	sourceBytes, err := json.Marshal(*source)
	if err != nil {
...
func (converter JsonMarshallingConverter[proto, db]) DbToProtoSlice(source []*db) (_ []*proto, err error) {
	defer func() {
		if r := recover(); r != nil {
	...
	return lo.Map(source, func(item *db, i int) *proto {
		x, err := converter.DbToProto(item)
		if err != nil {
			panic(err)
		}
		return x
...
func (converter JsonMarshallingConverter[proto, db]) ProtoToDbSlice(source []*proto) (_ []*db, err error) {
	defer func() {
		if r := recover(); r != nil {
	...
	return lo.Map(source, func(item *proto, i int) *db {
		x, err := converter.ProtoToDb(item)
		if err != nil {
